...
import 'package:drivers_app/helpers/mapkithelper.dart';
import 'package:drivers_app/widget/CollectPaymentDialog.dart';
...
class _NewTripPageState extends State<NewTripPage> {
  ...
  Future<void> getDirection(LatLng pickupLatLng, LatLng destinationLatLng) async{
    ...
    showDialog(
        ...
        context: context,
        builder: (BuildContext context) => ProgressDialog(status: 'fetching location..',)
    );
    ...
    setState(() {
      _circles.remove(pickupCircle);
      _circles.remove(destinationCircle);
    });
  ...
  void topUpEarnings(int fares){
    ...
    earningsRef.once().then((DataSnapshot snapshot) {
      ...
      if(snapshot.value != null){
        ...
        double adjustedEarnings = (fares.toDouble() * 0.85);/// so we taking 15% of Drivers earning that day and remit 85% to driver
        earningsRef.set(adjustedEarnings.toStringAsFixed(2));
      }
